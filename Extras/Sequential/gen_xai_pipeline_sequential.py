...
""" 
Generating keywords from images using QA model
"""
def split_image(image):
    patches = []
    width, height = image.size
    patch_width = width // sc.NUM_PATCHES
    patch_height = height // sc.NUM_PATCHES

    for i in range(sc.NUM_PATCHES):
        for j in range(sc.NUM_PATCHES):
            patch = image.crop((i * patch_width, j * patch_height, (i + 1) * patch_width, (j + 1) * patch_height))
            patches.append(patch)
    return patches

def get_answer(image, preprocessor, qa_model):
    patches = split_image(image)
    qas = []
    for patch in patches:
        qa = []
        for question in sc.QUESTIONS:
            inputs = preprocessor(patch, question, return_tensors="pt", padding=True).to(sc.DEVICE)
            outputs = qa_model.generate(**inputs, max_new_tokens=1000)
            answer = preprocessor.decode(outputs[0], skip_special_tokens=True)
            qa.append(answer)
        qas.append(qa)
    return qas

def generate_answers(image_folder_name, preprocessor, qa_model):
    img_list = os.listdir(os.path.join(sc.TRAIN_FOLDER, image_folder_name))
    output = {}
    k = 1
    for img in img_list:
        img_path = os.path.join(sc.TRAIN_FOLDER, image_folder_name, img)
        image = Image.open(img_path)
        answers = get_answer(image, preprocessor, qa_model)
        output[k] = answers
        k += 1
    return output

def combine_duplicates(keyword_freq):
    ...
    for word, count in keyword_freq:
        ...
        lemmatized_mapping[word] = lemmatized_phrase

    lemmatized_freq = defaultdict(int)

    for word, count in keyword_freq:
        lemmatized = lemmatized_mapping[word]
        lemmatized_freq[lemmatized] += count
    
    reverse_mapping = defaultdict(list)
    for orig, lemma in lemmatized_mapping.items():
        reverse_mapping[lemma].append(orig)

    reduced_keyword_freq = [(original, lemmatized_freq[lemma]) for lemma, original in reverse_mapping.items()]
    reduced_keyword_freq = sorted(reduced_keyword_freq, key=lambda x: x[1], reverse=True)

    # Replace the list of phrases with the first phrase in the list
    reduced_keyword_freq = [(phrases[0], count) for phrases, count in reduced_keyword_freq]

    return reduced_keyword_freq

def remove_elements_with_word(remove_word, keyword_freq):
    return [(phrase, count) for phrase, count in keyword_freq if remove_word.lower() not in phrase.lower()]

def generate_keywords_and_frequency(image_folder_name, preprocessor, qa_model, remove_word=None):
    answers = generate_answers(image_folder_name, preprocessor, qa_model)

    # Get keywords and frequency
    flaten_list = [item for sublist in sum(list(answers.values()), []) for item in sublist]
    keyword_freq = Counter(flaten_list)
    sorted_keyword_freq = sorted(keyword_freq.items(), key=lambda x: x[1], reverse=True)

    # Remove elements with word
    if remove_word:
        sorted_keyword_freq = remove_elements_with_word(remove_word, sorted_keyword_freq)

    # Combine duplicates
    reduced_keyword_freq = combine_duplicates(sorted_keyword_freq)

    # Split into keywords and frequency and return them
    keywords = [phrase for phrase, _ in reduced_keyword_freq]
    frequency = [count for _, count in reduced_keyword_freq]

    return keywords, frequency

preprocessor = BlipProcessor.from_pretrained(sc.QA_MODEL)
qa_model = BlipForQuestionAnswering.from_pretrained(sc.QA_MODEL).to(sc.DEVICE)
vit_model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14")
vit_processor = CLIPProcessor.from_pretrained("openai/clip-vit-large-patch14")

clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to(sc.DEVICE)
clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")

keywords, frequency = generate_keywords_and_frequency(sc.CLASS_NAME, preprocessor, qa_model, remove_word=sc.REMOVE_WORD)
...
with torch.no_grad():
    for word in words:
        inputs = clip_processor(text=word, return_tensors="pt", padding=True).to(sc.DEVICE)
...
# Sort the keywords based on vlm score
img_list = os.listdir(os.path.join(sc.TRAIN_FOLDER, sc.CLASS_NAME))

vlm_scores = []

for img in img_list:
    img_path = os.path.join(sc.TRAIN_FOLDER, sc.CLASS_NAME, img)
    image = Image.open(img_path)
    inputs = vit_processor(text=filtered_keywords, images=image, return_tensors="pt", padding=True)
    outputs = vit_model(**inputs)
    logits_per_image = outputs.logits_per_image
    probs = logits_per_image.softmax(dim=1)

    vlm_scores.append(probs)

# Combine the vlm scores with the keywords
with torch.no_grad():
    mean_score = torch.stack(vlm_scores).mean(dim=0).cpu().numpy()
mean_score = mean_score.tolist()[0]

combine = list(zip(filtered_keywords, mean_score))
combine = sorted(combine, key=lambda x: x[1], reverse=True)

keywords = [word for word, _ in combine]
if len(keywords) >= 20:
...
def get_reward(scores, layers, concepts):
    ...
    reward = scores[keys[0]][layers[0]]['sign_count'][0] #- scores[keys[0]][layers[0]]['sign_count'][1]
    return reward
...
class GenConceptEnv(gym.Env):
    ...
    def step(self, action):
        self.running += 1
        remove_cav_folder()
        self.past_action.append(actions[action])
        current_concept = " ".join(self.past_action)
...
dqn_model = DQN("MlpPolicy", env, verbose=1, buffer_size=100, exploration_final_eps=0.9, exploration_initial_eps=1.0)

rl_folder_name = f"saved_rl_models/dqn_models_{sc.DPO_WEIGHTS_PATH}"
os.makedirs(rl_folder_name, exist_ok=True)

checkpoint_callback = CheckpointCallback(save_freq=100, save_path=rl_folder_name, name_prefix="dqn_model")

dqn_model.learn(total_timesteps=sc.TOTAL_RL_STEP, callback=checkpoint_callback)
